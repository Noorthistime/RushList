...
export function TaskItem({ task, themeColor, onToggle, onDelete, onEdit }: TaskItemProps) {
  ...
  return (
    <motion.div
      ...
      style={style}
      className={`group flex flex-col md:flex-row md:items-center gap-2 p-3 md:gap-3 md:p-4 rounded-xl border border-border/50 bg-card/40 backdrop-blur-sm transition-all hover:bg-card/80 ${
        isDragging ? "shadow-xl border-primary" : "hover:border-border"
      }`}
    >
      {/* Mobile Top Row / Desktop Main Row */}
      ...
      {/* Mobile Bottom Row (Reminder aligned under title) */}
      {task.reminderTime && (
        <div className="w-full pl-8 pr-1 space-y-1.5 md:hidden">
          {renderReminder()}
        </div>
      )}
